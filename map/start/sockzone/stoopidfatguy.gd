...
func _ready():
	starting_position = position # so you can move it in the editor
	...
	while true:
		...
		# tweens the position variable to starting_position + Vector2(50, 0) over 3 seconds
		# tween instances cannot be reused once they have been started,
		# make sure to recreate them by doing `tween = create_tween()`!
		tween.tween_property(self, "position", starting_position + Vector2(50, 0), 3.0)
		...
		tween = create_tween()
		tween.tween_property(self, "position", starting_position, 3.0)
